...
JavaScript共有八种数据类型，分别是 Undefined Null Boolean  Number String Object Symbol  Bight

其中 Symbol 和 Bight 是 Es6中新增的数据类型：
+ Symbol 代表创建后独一无二且不可变的数据类型，它主要是为了解决可能出现的全局变量冲突的问题。
+ Bight 是一种数字类型的数据，它可以表示任意精度格式的整数，使用 Bight 可以安全地存储和操作大整数，即使这个数已经超出了 Number 能够表示的安全整数范围。
...
+ 引用数据类型存储在堆（heap）中的对象，占据空间大、大小不固定。如果存储在栈中，将会影响程序运行的性能；引用数据类型在栈中存储了指针，该指针指向堆中该实体的起始地址。当解释器寻找引用值时，会首先检索在栈中的地址，取得地址后从堆中获得实体。
...
+ 堆去内存一般由开发者分配释放，若开发者不释放，程序结束时可能由垃圾回收机制回收。
...
```js
  ...
  console.log(a.call(Symbol(1)));
  console.log(a.call(new Map()));
  console.log(a.call(new Set()));
```
